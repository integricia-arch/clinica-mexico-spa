...
2. ~~Aplicar 5 huecos de honorario pendientes~~ — **HECHO**, vía `contab_generar_poliza_evento` directo. Verificado: 7 movimientos honorario_appointment ↔ 7 pólizas, sin duplicados.
3. ~~Factura proveedor multi-línea~~ — **CERRADO por investigación**: `contab_diagnosticar_hueco()` excluye `factura_proveedor` del CASE a propósito (comentario en `20260721120000_corrector_huecos_diagnostico.sql` línea 20) — multi-línea (subtotal+IVA+proveedores) no mapea a cargo/abono simple. Confirmado 0 huecos reales de este tipo en prod hoy. No hay nada que corregir, es límite documentado.
4. ~~Farmacia COGS~~ — **HECHO**, migración `20260721190000_farmacia_cogs.sql`. `movimientos_inventario` ya traía `reference_type='pharmacy_sale'`+`lote_id`, y `lotes_medicamento.costo_unitario_centavos` ya existía — el gap de memoria técnica §5.1 estaba desactualizado. Se agregó: regla `costo_venta_farmacia` (502/115.02), trigger `contab_costo_venta_farmacia` en `movimientos_inventario`, y el cálculo en `pnl_mensual()`/`kpis_dashboard()`. Sin reversión automática en cancelación (mismo límite que ya tenía `consumo_insumo`, documentado). Advisors limpio.
5. ~~`honorario_pago_manual`~~ — **HECHO**. Investigación: era póliza de PRUEBA capturada a mano, sin trigger real. Se construyó el flujo real: tabla `honorario_pagos_manual`, RPC `honorarios_saldo_por_doctor()` + `honorario_registrar_pago()` (usa la regla `honorario_pago` ya existente desde antes, nunca disparada), tab "Honorarios" nueva en Contabilidad (`HonorariosTab.tsx`). Migración `20260721210000_honorario_pago_manual_flujo.sql` + fix `20260721211500_honorario_pago_manual_revoke_anon.sql` (anon tenía EXECUTE por default privilege de Supabase, revocado explícito). `tsc --noEmit` limpio.
6. ~~Paciente PRUEBA-E2E~~ — **HECHO**. El texto original ("falta Salida/alta 11/13") estaba desactualizado — `discharge` ya se había cerrado sesión 20-jul, dejando 12/13 con `current_step_key: followup`. Cuenta QA resultó sin `clinic_memberships`/`platform_staff` (no era el bloqueo de MFA de la sesión anterior, "Administrador" en el header es solo texto default sin membership real) — se cerró el step `followup` directo en BD replicando `saveJourneyStepData`+`syncFollowup`+`closeJourneyStep`+recálculo de `update_journey_progress` (esa RPC también le falta el bypass service_role, mismo patrón que tenía `crear_poliza()` — no corregido, fuera de alcance esta vez). Journey `f5bb9206-f134-4425-93a5-4f9961691123` → `completed_steps: 13/13`, `status: completado`. Cuenta QA quedó deshabilitada de nuevo al terminar.

Menor: dual-write de aprendizajes a `AGENTS.md` del proyecto sigue sin hacerse (solo `CLAUDE.md`) — pendiente por costo, no por decisión.

**Deuda nueva encontrada (no atacada, anotar para futuro):** `update_journey_progress(uuid)` tiene el mismo bug de diseño que tenía `crear_poliza()` antes del fix — exige `auth.uid()` real, sin bypass para llamadas service_role/cron. Si algún cron o Edge Function algún día necesita cerrar/recalcular un journey sin sesión de usuario, va a fallar igual. Revisar si aplica el mismo fix que se le dio a `crear_poliza()`.

Advisors de seguridad corridos 2 veces esta sesión (tras fix de `crear_poliza()` y tras cada migración nueva) — sin hallazgos nuevos relevantes, solo warnings genéricos preexistentes (GraphQL visibility, "authenticated puede ejecutar RPC").
...
- **Bloqueo real encontrado y resuelto:** el plan asumía botones "Ver trámite"/"Ver asiento contable" navegando a pantallas de detalle existentes por `id`. Verificado contra `App.tsx`: **no existen rutas `:id`** para cita/venta/compra individual — `/expedientes`, `/farmacia`, `/compras` son listas sin deep-link. Se decidió NO construir rutas nuevas — en vez de eso, `?highlight=<id>` sobre la lista existente (scroll + resalta la fila).
- **Fase 2 (hecho, commit `9cee001`):** `libro_diario()` ahora regresa `reference_id` (antes solo `reference_type`; de paso corrigió drift — `uuid_cfdi`/`reference_type`/`cuenta_tipo` ya vivían en prod sin migración commiteada). `PolizaDetalleDialog` agrega botón "Ver trámite"/"Ver cita"/"Ver venta"/"Ver compra" según `reference_type`, navega con `?highlight=`.
- **Fase 3 (hecho, commits `bfe29b7`, `a7b9c52`, `2b9ea74`, `676b215`):** RPC `poliza_detalle(poliza_id)` + componente compartido `VerAsientoContableButton.tsx` (resuelve `contab_resolver_asiento`, abre `PolizaDetalleDialog` si hay póliza, tarjeta simple si solo hay `movimiento_contable`, o "Sin asiento" si no hay nada). Cableado con 3 subagentes en paralelo:
  - **Citas.tsx**: "Ver cobro" (`consulta`) + "Ver honorario" (`honorario_appointment`, si `doctor_id`) por fila, resaltado `?highlight=`.
  - **Farmacia.tsx**: no había listado de ventas cableado en la pantalla — el agente agregó un tab nuevo "Historial" (`pharmacy_sales`) con el botón + resaltado. **Repasar este tab en browser primero**, es lo más nuevo/menos verificado.
  - **`src/features/compras/FacturasProveedor.tsx`** (no `Compras.tsx`, que solo monta `ComprasTabs`): botón + resaltado por factura.
  - `tsc --noEmit` limpio en las 4 tandas. **Verificado en browser, todo funciona:**
    - Bug crítico encontrado y corregido: `FacturasProveedor.tsx` importaba `VerAsientoContableButton` como default export (rompía TODA la app, pantalla en blanco en cualquier ruta) — el componente es named export. Fix: `import { VerAsientoContableButton }`.
    - Bug funcional encontrado y corregido: "Ver cobro" en Citas pasaba `reference_type="consulta"` + `appointment_id`, pero el trigger real guarda `reference_type='movimiento_caja'` + `reference_id=movimientos.id` (no el id de la cita) — nunca iba a resolver nada. Nueva RPC `contab_resolver_cobro_cita(appointment_id)` puentea desde la cita al movimiento de caja real. Migración `20260721170000_contab_resolver_cobro_cita.sql`, commit `de8a6fd`.
    - Confirmado en vivo: "Ver cobro" → póliza ING-1 real ($800, Caja/Ingresos consultas). "Ver honorario" → póliza folio 3 (misma cita). Farmacia → tab Historial carga ventas reales, botón resuelve limpio (sin/con póliza según antigüedad). Compras → póliza DIA-1 completa (3 partidas, subtotal+IVA+proveedores, cuadra) + link reverso "Ver compra".
...
- Migración `20260718110000_auto_user_provisioning.sql`: email en doctors/nurses,
  cola con clinic_id, `provision_link_user()` (rol + clinic_membership + user_id),
  trigger JIT en auth.users, RPC `provision_link_by_email`.
- Migración `20260718120000_provisioning_hardening.sql` (code review CRITICAL):
  JIT solo con `email_confirmed_at NOT NULL` (anti-secuestro de cuenta por signup
  con email ajeno); baja doctor/enfermera desactiva membership, reactivación la
  restaura; cola filtra por entity_type.
- Edge function `provision-users-from-queue` v3: fix `SUPABASE_ANON_KEY` sin
  definir (estaba rota en prod), password CSPRNG, vincula usuarios ya existentes,
  service_role bypass. `verify_jwt=true` en config.toml.
- AdminUsuarios: invoke revisa `{data,error}`, toast correcto, email lowercase.
- Flujo final: alta con email → cuenta Auth confirmada → login "Continuar con
  Google" auto-linkea → ve su clínica. Orden inverso (Google primero) también.
...
- Alan (alan.calderon.biomed@gmail.com) en cola: se vincula solo en su primer
  login Google, o al guardar cualquier alta en AdminUsuarios.
- Deuda LOW: función procesa toda la cola, toast no correlaciona entidad exacta.
- Recepcionistas siguen con staff_link_codes (scope aparte).
