...
| 5 | E1 tests CI (smoke E2E + RPCs contables) | Sonnet | ✅ HECHO (2026-07-22) — decisión tomada: NO montar pgTAP/Supabase-local en CI (infra grande para una función). En su lugar, `supabase/scripts/test_contab_generar_poliza_evento_MANUAL.sql`: test transaccional (BEGIN/ROLLBACK, nunca commitea) corrido vía MCP contra prod, cubre split de 3 líneas IVA fase 9, balance debe=haber, monto e IVA exactos. Corrido y verde, rollback verificado sin dejar rastro. Limitación documentada: corre como rol postgres/service (MCP), no reproduce el bug conocido de `crear_poliza()` bajo `authenticated`/`auth.uid()`. Re-correr a mano tras cualquier cambio a `contab_generar_poliza_evento`/`crear_poliza`/`contab_resolver_regla`. Smoke E2E sigue sin hacerse — fuera de esta sesión (costo) |
...
| 8 | E3 code-splitting + E2 partir archivos | Sonnet / Haiku | ✅ HECHO (verificado 2026-07-22) — E3: rutas lazy-loaded, bundle 3.38MB→676KB (bd3a2ed). E2 en CaminoPaciente/CajaTurno/BI/Expedientes/Pitch. AdminUsuarios.tsx también HECHO: 2038→1057 líneas, split en `src/pages/adminUsuarios/` (UsersTab, DoctorsTab, NursesTab, DoctorDialogs, NurseDialogs, UserDialogs, types) — commit `9a2d3d7`, ya estaba resuelto, STATE.md desactualizado |
...
| 16 | White-label multi-clínica: `logo_url`+`nombre` en `clinics`, wired en header/sidebar/PDFs/CFDI (reemplaza `Logo.tsx` hardcoded) | Sonnet | ✅ HECHO (2026-07-22) — `Logo.tsx` recibe `logoUrl`/`name`/`subtitle`, `AppLayout` usa clínica activa en sidebar. Nueva página `/configuracion/consultorio` (admin sube logo + edita nombre, bucket `clinic-logos` RLS por `clinic_memberships`). Ticket POS ya tomaba `activeClinic.name`, sin cambios. Migración `20260722040000_clinic_logos_bucket.sql` aplicada en prod vía MCP, advisors sin hallazgos nuevos. Commit c935543 (+ 23d0d60 de U3/U5/U6) |
