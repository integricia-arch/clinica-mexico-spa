...
-- SELECT policies used USING (true) — any authenticated user could read
-- any clinic's inventory data across tenants.
-- Applies the same clinic_id scoping pattern used in clinic_settings fix
-- (20260606183119): is_clinic_staff + user_has_clinic_access for SELECT,
-- and adds clinic_id guard to WRITE policies so admins only mutate their own clinic.

-- ─────────────────────────────────────────────────────────────
-- checklists
-- ─────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS "Authenticated can read checklists" ON public.checklists;
CREATE POLICY "Staff read own checklists"
...
CREATE POLICY "Staff read own proveedores"
...
CREATE POLICY "Staff read own insumos"
...
CREATE POLICY "Staff read own kits"
...
CREATE POLICY "Staff read own kit_items"
