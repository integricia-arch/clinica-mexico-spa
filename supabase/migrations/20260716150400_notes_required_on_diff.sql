...
-- Reescrita sobre la firma REAL de producción (5 params, p_supervisor_id) tal como quedó
-- tras supabase/scripts/diagnostics/_tmp_fix_autorizado_by.sql — la migración anterior
-- (bdde651) usaba por error la firma vieja de 4 params y creó un overload paralelo muerto
-- que dejaba sin protección el camino de cierre con override de supervisor.

-- 1. Elimina el overload de 4 params creado por error en bdde651
DROP FUNCTION IF EXISTS public.turno_close(uuid, numeric, text, boolean);

-- 2. Recrea la función real de 5 params con el check NOTES_REQUIRED_ON_DIFF agregado
...
BEGIN
  ...
  -- Explicación obligatoria cuando hay diferencia (gap 3) — ANTES del umbral/override
  -- para que un supervisor no pueda saltarse el requisito de notas.
