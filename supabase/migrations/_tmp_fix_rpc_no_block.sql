-- Fix pharmacy_register_sale:
--   1. NEVER block for prescription/controlled meds (avisa pero procede)
--   2. Log all exceptions to pos_error_logs before re-raising
...
BEGIN
  ...
  -- Validate items (lote existence/stock/expiry) — NO blocking on prescription requirement
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- RULE: never block sale for prescription/controlled requirement.
    -- Frontend shows modal to capture data; RPC trusts operator decision.
...
EXCEPTION WHEN OTHERS THEN
  -- Always log exceptions to pos_error_logs before re-raising
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    NULL; -- never let logging failure hide the original error
  END;
  RAISE;
END;
$function$;
