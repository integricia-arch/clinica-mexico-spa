-- Fix: recepcion_revertir() era SECURITY DEFINER sin GRANT explícito (quedaba
-- ejecutable por PUBLIC por default) y sin validar que el caller pertenezca a
-- la clínica dueña de la recepción. Cualquier usuario autenticado podía borrar
-- la recepción/factura y revertir inventario de OTRA clínica con solo el UUID.

REVOKE EXECUTE ON FUNCTION public.recepcion_revertir(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recepcion_revertir(uuid) TO authenticated;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'No autorizado para revertir recepciones de esta clínica';
  END IF;
