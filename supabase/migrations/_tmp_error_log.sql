-- Tabla de logs de error para debugging
CREATE TABLE IF NOT EXISTS public.pos_error_logs (
...
GRANT INSERT, SELECT ON public.pos_error_logs TO authenticated;
GRANT ALL ON public.pos_error_logs TO service_role;
ALTER TABLE public.pos_error_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admin lee logs" ON public.pos_error_logs;
CREATE POLICY "Admin lee logs" ON public.pos_error_logs
...
-- Función con manejo de errores y logging automático
...
BEGIN
  ...
  VALUES (v_user, 'crear', 'pharmacy_sales', v_sale_id,
          jsonb_build_object('event','pharmacy_sale_created','sale_type',v_sale_type,
                             'total',v_total,'shift_id',v_shift_id), v_clinic);

  RETURN v_sale_id;

EXCEPTION WHEN OTHERS THEN
  -- Log automático del error con contexto completo
