-- Fix turno_corte_x y turno_close:
-- Si el turno tiene pharmacy_shift_id → incluir cobros de pharmacy_sale_payments
-- (pharmacy_sales.shift_id = turnos.pharmacy_shift_id, payment_method='efectivo', status='completed')
...
BEGIN
  ...
  -- Cobros efectivo vía movimientos generales
  ...
  -- Si turno vinculado a farmacia, sumar cobros de pharmacy_sale_payments
  IF v_turno.pharmacy_shift_id IS NOT NULL THEN
    ...
    WHERE ps.shift_id = v_turno.pharmacy_shift_id
    ...
    WHERE shift_id = v_turno.pharmacy_shift_id AND status = 'completed';
  ELSE
    SELECT COUNT(*)::integer INTO v_ticket_count
  ...
  INSERT INTO public.cortes (
    ...
    jsonb_build_object(
      ...
      'cash_pharmacy', v_cash_pharmacy,
      'fondos_net', v_fondos_net,
      'expected', v_expected,
      'tickets', v_ticket_count,
      'is_pharmacy_turno', (v_turno.pharmacy_shift_id IS NOT NULL)
    )
...
-- Misma corrección para turno_close
