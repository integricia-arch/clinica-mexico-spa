-- supabase/migrations/20260625000010_pharmacy_register_sale_loyalty_cap.sql
-- Añade cap a v_loyalty_discount para que no exceda (subtotal - discount).
-- También añade GREATEST(0,...) a v_total para evitar totales negativos.
...
BEGIN
  ...
  -- Limitar loyalty_discount para no exceder lo que se puede descontar
  v_loyalty_discount := GREATEST(0, LEAST(v_loyalty_discount, GREATEST(0, v_subtotal - v_discount)));
  -- total incluye descuento manual y descuento por lealtad
  v_total := GREATEST(0, v_subtotal - v_discount - v_loyalty_discount);
