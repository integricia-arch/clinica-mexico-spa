-- supabase/migrations/20260723130000_trazabilidad_raiz_orden_fallback.sql
-- Fix: _contab_trazar_raiz debe seguir la cadena hasta orden_compra cuando
-- factura_proveedor.solicitud_id es NULL (flujo de compra directa sin solicitud
-- previa, ver contab_trazar_proveedor en 20260723120000_trazabilidad_proveedor.sql).
-- Sin este fallback, la trazabilidad se detiene en factura_proveedor perdiendo
-- los ancestros orden_compra/recepcion_mercancia.
--
-- Cleanup: _contab_trazar_nodo declaraba v_poliza record sin usar; se elimina.
  ...
  LOOP
    ...
    CASE v_tipo
      ...
      WHEN 'factura_proveedor' THEN
        SELECT solicitud_id, orden_id INTO v_next_id, v_orden_fallback
        FROM public.facturas_proveedor fp WHERE fp.id = v_id;
        IF v_next_id IS NOT NULL THEN
          v_next_tipo := 'solicitud_compra';
        ELSIF v_orden_fallback IS NOT NULL THEN
          v_next_id := v_orden_fallback;
          v_next_tipo := 'orden_compra';
        END IF;
