...
BEGIN
  ...
  PERFORM id FROM public.lotes_medicamento
    WHERE id IN (
      SELECT COALESCE(
        NULLIF(_elem->>'lote_id', '')::uuid,
        (SELECT id FROM public.lotes_medicamento lm2
         WHERE lm2.medicamento_id = (_elem->>'medicamento_id')::uuid
           AND lm2.existencia >= COALESCE((_elem->>'quantity')::int, 1)
           AND lm2.fecha_caducidad >= CURRENT_DATE
         ORDER BY lm2.fecha_entrada ASC, lm2.fecha_caducidad ASC, lm2.id ASC LIMIT 1)
      )
      FROM jsonb_array_elements(p_payload->'items') AS _elem
    )
  ORDER BY id
  FOR UPDATE;
