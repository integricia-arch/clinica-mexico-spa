...
BEGIN
  ...
  -- clinic_id viene del payload (enviado por el frontend)
  v_clinic := NULLIF(p_payload->>'clinic_id', '')::uuid;
  IF v_clinic IS NULL THEN
    -- fallback: clínica del paciente
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    IF v_sale_type = 'direct_sale' THEN
      -- controlado (column name in actual schema)
      ...
      END IF;
      -- requiere_receta (column name in actual schema)
    ...
    -- FIFO: lote más próximo a vencer con suficiente existencia
