...
-- Índices faltantes identificados en audit.
-- CONCURRENTLY removido: Supabase CLI no lo soporta (pipeline/transacción).
-- Lock breve en tablas pequeñas de clínica (~ms).

BEGIN;

-- appointment_resources: FK sin index
CREATE INDEX IF NOT EXISTS idx_appointment_resources_appointment
  ON public.appointment_resources(appointment_id);

-- pharmacy_sale_items: FK sin index
CREATE INDEX IF NOT EXISTS idx_psi_medicamento
  ON public.pharmacy_sale_items(medicamento_id);

CREATE INDEX IF NOT EXISTS idx_psi_lote
  ON public.pharmacy_sale_items(lote_id)
  WHERE lote_id IS NOT NULL;

-- notas_consulta: FK sin index
CREATE INDEX IF NOT EXISTS idx_notas_doctor
  ON public.notas_consulta(doctor_id);

CREATE INDEX IF NOT EXISTS idx_notas_appointment
...
CREATE INDEX IF NOT EXISTS idx_appointments_clinic_doctor_fecha
...
DROP INDEX IF EXISTS public.idx_medicamentos_activo;
CREATE INDEX IF NOT EXISTS idx_medicamentos_clinic_activo
...
CREATE INDEX IF NOT EXISTS idx_chat_pendientes_pregunta_trgm
