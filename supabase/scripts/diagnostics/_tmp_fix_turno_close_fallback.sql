-- Fix: turno_close agregar fallback pharmacy_cash_shifts cuando pharmacy_shift_id IS NULL
-- Problema: turnos con pharmacy_shift_id=NULL no sumaban cobros farmacia en Corte Z
-- Solución: buscar último pharmacy_cash_shift del cajero si pharmacy_shift_id es NULL
-- También se eliminó overload viejo (p_contado, p_notas) que estaba huérfano

DROP FUNCTION IF EXISTS public.turno_close(uuid, numeric, text);
...
BEGIN
  ...
  -- Resolver pharmacy shift: directo o fallback por cajero
