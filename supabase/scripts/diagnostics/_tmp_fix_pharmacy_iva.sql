-- Fix: pharmacy_register_sale calcular y persistir IVA por item y total
-- Problema: total_iva en pharmacy_sales siempre quedaba en 0
--           base_imponible e iva_amount en pharmacy_sale_items nunca se guardaban
-- Solución: calcular IVA desde medicamentos.tasa_iva (precios incluyen IVA)
--           base = round(subtotal_item / (1 + tasa_iva), 2)
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- IVA desglose: precios incluyen IVA, extraer base e IVA
    v_item_base := round(v_isubtotal / (1 + COALESCE(v_med.tasa_iva, 0)), 2);
    v_item_iva  := v_isubtotal - v_item_base;
    v_total_iva := v_total_iva + v_item_iva;
    ...
    VALUES
      (v_sale_id, v_clinic, v_med.id, v_pick_lote,
       ...
       v_qty, v_unit, v_idisc, v_isubtotal, v_item_base, v_item_iva);
  ...
  VALUES (v_user, 'crear', 'pharmacy_sales', v_sale_id,
          jsonb_build_object('event','pharmacy_sale_created','sale_type',v_sale_type,
                             ...
                             'shift_id',v_shift_id,'receta_capturada',v_receta_capturada), v_clinic);
