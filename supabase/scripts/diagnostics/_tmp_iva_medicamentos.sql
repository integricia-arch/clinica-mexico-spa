-- Task 3: IVA modeling per medicamento
-- Medicamentos con receta/controlados → IVA tasa 0 (exentos SAT)
-- OTC → IVA 16%

-- 1. Agregar tasa_iva a medicamentos
ALTER TABLE public.medicamentos
  ADD COLUMN IF NOT EXISTS tasa_iva numeric(4,2) NOT NULL DEFAULT 0.16;

-- 2. Medicamentos con receta o controlados: tasa 0
UPDATE public.medicamentos
   SET tasa_iva = 0.00
 WHERE requiere_receta = true OR controlado = true;

-- 3. Agregar columnas IVA a pharmacy_sale_items
...
-- 4. Agregar total_iva a pharmacy_sales
ALTER TABLE public.pharmacy_sales
  ADD COLUMN IF NOT EXISTS total_iva numeric(12,2) NOT NULL DEFAULT 0;

-- 5. Reemplazar RPC con cálculo IVA por ítem
...
BEGIN
  ...
  IF v_sale_type NOT IN ('direct_sale','prescription_dispense') THEN
    RAISE EXCEPTION 'sale_type inválido: %', v_sale_type;
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- IVA desglosado: precio_unitario ya incluye IVA (precio con IVA)
    -- base = subtotal / (1 + tasa_iva)
    -- iva  = subtotal - base
    IF v_med.tasa_iva > 0 THEN
      v_ibase := ROUND(v_isubtotal / (1 + v_med.tasa_iva), 2);
      v_iiva  := v_isubtotal - v_ibase;
    ELSE
      v_ibase := v_isubtotal;
      v_iiva  := 0;
    END IF;
    ...
    INSERT INTO public.pharmacy_sale_items
      (sale_id, clinic_id, medicamento_id, lote_id, prescription_item_id,
       quantity, unit_price, discount, subtotal, base_imponible, iva_amount)
    VALUES
      (v_sale_id, v_clinic, v_med.id, v_pick_lote,
       NULLIF(v_item->>'prescription_item_id','')::uuid,
       v_qty, v_unit, v_idisc, v_isubtotal, v_ibase, v_iiva);
    ...
    v_subtotal   := v_subtotal + v_isubtotal;
    v_total_iva  := v_total_iva + v_iiva;
  END LOOP;

  v_total := v_subtotal - v_discount;
  UPDATE public.pharmacy_sales
     SET subtotal = v_subtotal, total = v_total, total_iva = v_total_iva
   WHERE id = v_sale_id;

  INSERT INTO public.audit_logs (user_id, accion, tabla, registro_id, datos_nuevos, clinic_id)
  VALUES (v_user, 'crear', 'pharmacy_sales', v_sale_id,
          jsonb_build_object('event','pharmacy_sale_created','sale_type',v_sale_type,
                             'total',v_total,'total_iva',v_total_iva,
