// =================================================================
// supabase/functions/telegram-webhook/index.ts  (v4)
//
...
4. Presenta 3-4 opciones en formato humano: "jueves 6 de mayo a las 10:30 am". Espera elección.
5. Recolecta datos UNO A UNO:
   ...
   Usa guardar_borrador_paciente cada vez que el paciente comparte un dato.
...
async function manejarMensaje(chatId: string, rawMsg: any, text: string) {
  ...
  // No se necesita cambio de schema: obtenerOCrearConversacion solo busca
  // "activa", así que la nueva conv toma precedencia automáticamente.
  if (text === "/nueva") {
  ...
  // Después de /nueva se inserta una conv activa con created_at mayor,
  // por lo que supera a cualquier conv escalada previa en el ORDER BY.
...
async function listarHorariosDisponibles({ servicio_id, dias_adelante = 7 }: any) {
  ...
  const horarios: Array<{ doctor_id: string; doctor_nombre: string; fecha_inicio: string }> = [];

  for (let d = 0; d < dias_adelante && horarios.length < 12; d++) {
    const dia = new Date(ahora.getTime() + d * 86400000);
    if (!DIAS_LABORALES.includes(dia.getDay())) continue;
    ...
    for (const doc of doctores) {
      ...
      const inicioDia = new Date(dia); inicioDia.setHours(sh, sm, 0, 0);
      const finDia    = new Date(dia); finDia.setHours(eh, em, 0, 0);
...
// Solo busca convs ACTIVAS. El check de "escalada" ocurre antes en
// manejarMensaje. Así se evita el bug del ciclado original.
